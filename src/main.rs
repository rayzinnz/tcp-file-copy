...
use tcp_file_copy::{DownloadClientInitalise, DownloadServerInitalise, FileCopyOperation, FileCopyStep, SIGNATURE, StreamProgress, send_file_to_host};
        ...
        Ok(n) if n >= 6 => {
            ...
            if is_upload == 0 {
                ...
                if step == FileCopyStep::Initialise {
                    ...
                    let download_client_initialise:DownloadClientInitalise = wincode::deserialize(stream_bytes).expect("Could not deserialize bytes to StreamProgress");
                    debug!("{:#?}", download_client_initialise);
                    let full_path: PathBuf;
                    match root_path {
                        Some(root_path) => {
                            full_path = absolute(root_path.join(&download_client_initialise.serverside_path))?;
                        }
                        None => {
                            full_path = PathBuf::from(&download_client_initialise.serverside_path);
                        }
                    }
...
fn print_usage() {
    ...
    // cargo run send_file XXPA201LAP00072.local 52709 "./tests/Bremshley Treadmill Service Manual.pdf" "./large"
    ...
    // cargo run get_file XXPA201LAP00072.local 52709 "./large/Bremshley Treadmill Service Manual.pdf" "C:\Users\hrag\temp\rec"
    eprintln!("\nExample:");
